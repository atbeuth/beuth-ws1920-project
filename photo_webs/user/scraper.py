...
class Media:
    def __init__(self, media_node):
        ...
        try:
            self.title = media_node["edge_media_to_caption"]["edges"][0]["node"][
                "text"
            ].replace('"', '*/?"')
        except Exception:
...
class InstagramImageScraper:
    ...
    def get_shared_data(self, url):
        html = (
            urllib.request.urlopen(url, context=self.ctx)
            .read()
            .decode("utf-8", "ignore")
        )
        ...
        if script:
            ...
            while has_next_page:
                ...
                try:
                    ...
                    new_variables = urllib.parse.quote_plus(
                        self.media_variables % (profile.id, end_cursor)
                    )
                    new_url = self.graph_ql_url % (self.media_query_hash, new_variables)
                    print("\nLoading next page ...")
                    media_response = (
                        urllib.request.urlopen(new_url, context=self.ctx)
                        .read()
                        .decode("utf-8")
                    )
                    media_json = json.loads(media_response)

                    has_next_page = media_json["data"]["user"][
                        "edge_owner_to_timeline_media"
                    ]["page_info"]["has_next_page"]
                    end_cursor = media_json["data"]["user"][
                        "edge_owner_to_timeline_media"
                    ]["page_info"]["end_cursor"]

                    edges = media_json["data"]["user"]["edge_owner_to_timeline_media"][
                        "edges"
                    ]
                    current_size = len(data["media"])
                    for i, media_edge in enumerate(edges, start=1):
                        print(
                            "\rProcess: %d von %d" % (current_size + i, total_media),
                            end="",
                        )
                        try:
                            picture = Media(media_edge["node"])
