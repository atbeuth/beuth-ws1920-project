...
def is_image_in_search(search, image):
    ...
    for term in search.split(" "):
        if (
            (term.upper() in image.tags.upper())
            or (term.upper() in image.title.upper())
            or (term.upper() in image.description.upper())
            or (term.upper() in image.long_description.upper())
            or (term.upper() in image.category.upper())
            or (term.upper() in image.user.username.upper())
        ):
            return True
    return False
...
def is_user_in_search(search, user):
    for term in search.split(" "):
        if (
            (term.upper() in user.username.upper())
            or (term.upper() in user.profile.bio.upper())
            or (term.upper() in user.profile.bio_short.upper())
        ):
            return True
    return False
...
def to_insta_post(poststring):
    ...
    for current_post_string in post_list_choices:
        try:
            current_post_json = json.loads(
                current_post_string.replace("[", "")
                .replace("'", '"')
                .replace('*/?"', "'")
                .replace("True", "true")
                .replace("False", "false")
